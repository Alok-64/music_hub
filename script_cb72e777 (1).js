...
// Spotify Downloader API configuration
const SPOTIFY_API_KEY = '83fffa8307msh28731e855006619p1eb69cjsna2d9f63e95cb';
const SPOTIFY_API_HOST = 'spotify-downloader9.p.rapidapi.com';
const SPOTIFY_API_BASE = 'https://spotify-downloader9.p.rapidapi.com';
...
async function fetchDownloadLink(spotifyTrackId) {
  ...
  const res = await fetch(
    `${SPOTIFY_API_BASE}/downloadSong?songId=${encodeURIComponent(songUrl)}`,
    {
      headers: {
...
async function searchSongsAPI(query) {
  ...
  // Use Spotify Downloader API search for full catalog with download capability
  return spotifySearch(query);
}

async function spotifySearch(query) {
  const endpoint = `${SPOTIFY_API_BASE}/search?q=${encodeURIComponent(query)}`;
  try {
    ...
    showNotification('Search failed. CORS error? This app must be hosted on a web server to work.');
    renderSearchResults([]);
    return [];
  }
}
...
function playPreview(audioUrl, title, artist, artworkUrl, fullTrackUrl, songstatsId, spotifyId = '') {
  ...
  // Show download button if we have a Spotify ID or a direct download URL
  ...
  if (spotifyId) {
    ...
    // Try autoplay; if it fails, show a prominent play button for user interaction
...
if (playerDownloadBtn) {
  playerDownloadBtn.addEventListener('click', () => {
    if (currentSpotifyId) {
      // Use Spotify Downloader API for full track download
